...
$(document).ready(function() {
  ...
  var table = $('#bookReturnTable').DataTable({
        data: JSON.parse($('#bookReturnData').html()),
        'bSort': true,
        "pageLength": 25,
        columns: [
          ...
          { data: 'actualdaysleft', title:'Days Left', defaultContent:'N/A'},
          { data: 'fine', title:'Fine', defaultContent:'N/A'},
          { data: null, title:'Return', defaultContent:'<i class="fa  fa-check-circle return-datatable-action"></i>'},
        ],
        columnDefs: [
          { "width": "5%", "targets": 0 },
          { "width": "20%", "targets": 1 },
  ...
  $('.return-datatable-action').click(function(){
    var child = $(this).parent().parent().children()[0];
    var rowId = child._DT_CellIndex.row;
    var data = JSON.parse($('#bookReturnData').html());
    // alert(JSON.stringify(data[rowId]));
    if(confirm("Press Ok to confirm return for following book. \n\nTitle"+data[rowId].title+" \n\nBorrow Date:"+data[rowId].borrowdate+"\nFine: $"+parseFloat(data[rowId].fine).toFixed(2)+"\nISBN: "+data[rowId].isbn))
    {
      $.ajax({
            url: "/bookReturn",
            type: 'PUT',
